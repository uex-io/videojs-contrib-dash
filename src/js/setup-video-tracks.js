...
// https://html.spec.whatwg.org/multipage/media.html#dom-videotrack-kind
  ...
  function generateKindFromTrack(track) {
    ...
    // TBD: need confirm how accessability options might affect track kind for video
    let roles = track.roles;
    if (roles) {
      // main combinations
      if (roles.some(role => role === "main")) {
        if (roles.some(role => role === "caption")) {
          return "captions";
        } else if (roles.some(role => role === "subtitle")) {
          return "subtitle";
        } else {
          return "main";
        } 
      ...
      if (roles.some(role => role === "commentary")) {
        return "commentary";
      }
